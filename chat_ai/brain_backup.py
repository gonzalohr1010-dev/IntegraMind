...
class Brain:
    def __init__(self, model_name: str = "all-MiniLM-L6-v2", storage_dir: Optional[str] = None, prefer_faiss: bool = True, user_id: str = "anonymous", db_path: str = None) -> None:
        # Initialize embedder first (needed for enhanced memory)
        try:
        ...
        try:
            ...
            # Use provided db_path or default to storage_dir
            memory_db = db_path or (os.path.join(storage_dir, 'memory.db') if storage_dir else 'memory.db')
            ...
            logger.info(f"Enhanced memory initialized for user {user_id}")
        except Exception as e:
            logger.warning(f"Failed to initialize enhanced memory, falling back to simple memory: {e}")
            # Fallback to simple memory
            self.enhanced_memory = ChatMemory(max_chars=4000)
        
        # Keep old memory for backwards compatibility (will be deprecated)
        self.memory = self.enhanced_memory if hasattr(self.enhanced_memory, 'add') else ChatMemory(max_chars=4000)
        ...
        # Initialize Domain Classifier (Universal Platform)
    ...
    def ask(self, question: str, top_k: int = 4, tone: str | None = None, prefs: Dict[str, str] | None = None, stream: bool = False, conversation_id: Optional[int] = None) -> Union[Dict[str, object], Tuple[Iterator[str], List[Dict[str, str]]]]:
        ...
        # --- CORE RETRIEVAL (Moved up for Reality Weaver) ---
        # We retrieve early so we can detect Experience Objects (Projections) even if tools are used.
        ...
        # --- DOMAIN DETECTION (Universal Platform) ---
        # Detect the domain of the user's query to apply domain-specific configurations
        detected_domain = self.domain_classifier.classify(question, context={'previous_domain': getattr(self, '_last_domain', None)})
        self._last_domain = detected_domain  # Track for context continuity
        domain_config = self.domain_classifier.get_domain_config(detected_domain)
        logger.info(f"Detected domain: {detected_domain} {domain_config.get('icon', '')}")
        
        # Inject domain info into projection if it exists
        if projection and 'domain' not in projection:
            projection['domain'] = detected_domain
            projection['domain_config'] = domain_config
        ...
        if tool_needed:
            ...
            if tool_needed in ['calculator', 'datetime', 'system_info']:
                ...
                if stream:
                    def _gen_tool():
                        yield answer
                    return _gen_tool(), []
                return {"answer": answer, "references": [], "projection": projection}
            ...
            # Case B: Context tools (web search, python analysis) -> Synthesize answer
                ...
                # Try to inject media if not present, even if tool was used
                if not projection:
                    projection = self._inject_dynamic_media(question, detected_domain)
                    if projection:
                        projection['domain'] = detected_domain
                        projection['domain_config'] = domain_config
        ...
        # retrieved = self.retrieve(question, top_k=top_k) # Already retrieved at start
        ...
        # Build system prompt based on domain persona
        persona = domain_config.get('persona', 'Universal Assistant')
        instruction = domain_config.get('system_instruction', 'You are a helpful assistant.')
        
        system = (
            f"ROLE: {persona}\n"
            f"INSTRUCTION: {instruction}\n"
            f"TONE: {tone_text}\n"
            f"USER PREFERENCES: {extra_prefs}\n\n"
            "CORE RULES:\n"
            "1. Use the provided CONTEXT to answer. If the answer is not in the context, say 'I don't have that information' unless it's general knowledge.\n"
            "2. Be professional and precise according to your role.\n"
            "3. Cite references using [n] if applicable.\n"
            "4. Respond in the language of the user (Spanish/English).\n"
            "5. YOU HAVE VISUALIZATION CAPABILITIES. If the user asks to 'show', 'visualize', or 'see' something, assume the system will display it. Do NOT say you cannot show images. Instead, say 'Here is the visualization you requested' or describe what is being shown."
        )
        ...
        # Check for Experience Objects in retrieved docs to return as "projection"
        # (Already calculated at start of method)

        # --- DYNAMIC MEDIA INJECTION ---
        if not projection:
            logger.info(f"Checking dynamic media for: '{question}' in domain '{detected_domain}'")
            projection = self._inject_dynamic_media(question, detected_domain)
            if projection:
                logger.info(f"DYNAMIC MEDIA INJECTED: {projection.get('title')}")
                # Inject domain info
                projection['domain'] = detected_domain
                projection['domain_config'] = domain_config
            else:
                logger.info("DYNAMIC MEDIA: No injection triggered (keywords not found or error)")

        logger.info(f"FINAL RETURN: projection={'FOUND' if projection else 'None'}, retrieved={len(retrieved)}")
        return {
            "answer": answer,
    ...
    def _inject_dynamic_media(self, question: str, domain: str) -> Optional[Dict]:
        """
        Analyzes if the question warrants a visual aid and generates/retrieves it.
        Returns a media asset object or None.
        """
        # Simple keyword heuristic for now, can be LLM-based later
        visual_keywords = ['show me', 'look like', 'visualize', 'diagram', 'image', 'photo', 'picture', 
                          'muéstrame', 'ver', 'imagen', 'foto', 'diagrama', 'design', 'diseña', 'draw', 'dibuja']
        
        needs_visual = any(kw in question.lower() for kw in visual_keywords)
        ...
        if needs_visual:
            try:
                ...
                # In a real app, these should be shared instances
                # For this demo, we create a lightweight instance or mock
                
                # Generate a prompt based on domain
                prompt = f"{domain} visualization: {question}"
                
                # Create a dynamic "Experience" object
                return {
                    "title": f"Visualización: {domain.capitalize()}",
                    "type": "generated_content",
                    "domain": domain,
                    "media_assets": [
                        {
                            "type": "image",
                            "url": f"/static/assets/placeholder_{domain}.png", # Placeholder
                            "caption": f"Visualización generada para: {question}",
                            "prompt": prompt
                        }
                    ],
                    "steps": [question],
                    "sensory_data": {
                        "Visual": "Generado dinámicamente",
                        "Contexto": domain.capitalize()
                    }
                }
            except Exception as e:
                logger.error(f"Media injection failed: {e}")
                return None
        return None
